...
def addingUsers(input):
    try:
        sql = "INSERT into bot.users(UserID, UserName, notifiable) VALUES (%s, %s, %s)"
        val = input
...
def addTransaction(input):
    try:
        sql = "INSERT into bot.transactions(transaction_id, OrderID, date, AmountOwed, UserID_Creditor, UserID_Debitor) VALUES (%s, %s, %s, %s, %s, %s)"
        val = input
        mycursor.execute(sql,val)
        mysqldb.commit()
        print('Records inserted successfully!')
...
def addOrder(input):
    try:
        sql = "INSERT into bot.orders(OrderID, GroupID) VALUES (%s, %s)"
        val = input
        mycursor.execute(sql,val)
        mysqldb.commit()
        print('Records inserted successfully!')
...
def addGroup(input):
    try:
        sql = "INSERT into bot.groups(GroupID, Number_of_members) VALUES (%s, %s)"
        val = input
        mycursor.execute(sql,val)
        mysqldb.commit()
        print('Records inserted successfully!')
