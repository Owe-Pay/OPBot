...
from ..owepaybot import newOrderSplitUnevenly
...
class TestNewOrderSplitEvenly:
    ...
    def test_newOrderSplitEvenly(self, orderUpdate):
        ...
        assert updateOrderIDToUserGroupRelational(456, 345, "tempOrderName")
        message = newOrderSplitUnevenly(orderUpdate, tempContext)
        assert isinstance(message, Message)
        assert userStateSplitUnevenly(456, 345) == True
        orderID = getOrderIDFromUserIDAndGroupID(456, 345)
        assert getOrderNameFromOrderID(orderID) == "tempOrderName"
        assert message.chat_id == 345
        assert message.message_id == 234
        assert message.text == "Please send in the items in the following format:\nItem Name - Price\n\nFor example:\nChicken Rice - 5\nCurry Chicken - 5.50\nNasi Lemak - 4"
